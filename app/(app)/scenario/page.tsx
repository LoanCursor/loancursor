 ...
 *
 * (MI omitted; most DSCR products do not have MI.)
 */
function calcDSCR({
  rentMonthly,
  taxAnnual,
  ...
  const pitia =
    (paymentMonthly || 0) + taxesMonthly + hoiMonthly + (hoaMonthly || 0);
...
export default function ScenarioPage() {
  ...
  function update<K extends keyof Scenario>(key: K, value: Scenario[K]) {
    setScenario((s) => {
      ...
      // --- Recompute DSCR immediately so it stays current while typing ---
      ...
      next.dscr = calcDSCR({
        ...
        hoiAnnual: (next as any).hoiAnnual ?? 0, // expects you add hoiAnnual to Scenario
  ...
  // Compute DSCR + PI + PITIA (read-only display)
  ...
  const pitia = React.useMemo(() => {
    return payment + taxesMonthly + hoiMonthly + (scenario.hoaMonthly || 0);
  ...
  const computedDscr = React.useMemo(() => {
    return calcDSCR({
      ...
      hoiAnnual: (scenario as any).hoiAnnual || 0,
  ...
  React.useEffect(() => {
    const rounded = Number.isFinite(computedDscr)
      ? Math.round(computedDscr * 100) / 100
      : 0;
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground leading-relaxed max-w-2xl">
            Organized inputs by section. Run pricing to evaluate eligibility and
            rule-based pricing across your lender set.
          </p>
        </div>
      ...
      <Card className="mt-6 rounded-xl border border-border bg-card p-6">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  {scenario.propertyType === "Condo" ? (
                    <Field label="Condo">
                      <Select
                        ...
                        onValueChange={(v) =>
                          update("condoWarrantable", v === "Warrantable" as any)
                        }
                      >
                        <SelectTrigger className={inputBase}>
                          <SelectValue placeholder="Select" />
                  ...
                  <Field label="Lien position">
                    <Select
                      ...
                      onValueChange={(v) => update("lienPosition", v as any)}
                    >
                      <SelectTrigger className={inputBase}>
                        <SelectValue placeholder="Select" />
                      </SelectTrigger>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  <Field label="Property value">
                    <CurrencyInput
                      ...
                      onChange={(n) => {
                        lastEdited.current = "loanAmount"; // doesn’t matter; keeps sync stable
                  ...
                  <Field label="Loan amount">
                    <CurrencyInput
                      ...
                      onChange={(n) => {
                        lastEdited.current = "loanAmount";
                        update("loanAmount", n as any);
                      }}
                    />
                  </Field>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <Field label="Prepay type">
                    <Select
                      ...
                      onValueChange={(v) => update("prepayType", v as any)}
                    >
                      <SelectTrigger className={inputBase}>
                        <SelectValue placeholder="Select" />
                      </SelectTrigger>
                    ...
                    <div className="space-y-2 rounded-md border border-border bg-background px-3 py-2.5">
                      <label className="flex items-center gap-2">
                        <Checkbox
                          ...
                          onCheckedChange={(v) => update("isSTR", !!v as any)}
                        />
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <Field label="Annual property taxes">
                    <CurrencyInput
                      ...
                      onChange={(n) => update("propertyTaxAnnual", n as any)}
                    />
                  </Field>
                  ...
                  <Field label="Qualifying rate">
                    <PercentInput
                      ...
                      onChange={(n) => update("qualifyingRate", n as any)}
                      className={inputBase}
                    />
                  </Field>

                  {/* ✅ Better DSCR card with PI + PITIA */}
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
                        <div>
                          ...
                          <div className="tabular-nums text-foreground/90">
                            {Number.isFinite(payment) ? `$${payment.toFixed(2)}` : "--"}
                          </div>
                        </div>
                        <div>
                          <div className="uppercase tracking-wide">PITIA</div>
                          <div className="tabular-nums text-foreground/90">
                            {Number.isFinite(pitia) ? `$${pitia.toFixed(2)}` : "--"}
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    ...
                    <span className="text-foreground">all lenders</span> or only
                    a selected subset.
                  </p>
                </div>
        ...
        <div className="pt-6 text-xs text-muted-foreground">
          Phase 1 is deterministic rules (no AI, no uploads). AI can assist
          maintainers later, but rules stay the authority.
        </div>
      </Card>
    </div>
  );
